export async function onRequest(context) {
  ...
  const [online, dau, totalPlayers, totalGames, avgSurvival, regionResults] = await Promise.all([
    ...
    db.prepare(`SELECT country, region, city, COUNT(DISTINCT player_id) AS count FROM heartbeat WHERE country IS NOT NULL AND seen_at >= datetime('now', '-7 days') GROUP BY country, region, city ORDER BY count DESC LIMIT 20`).all(),
  ]);
